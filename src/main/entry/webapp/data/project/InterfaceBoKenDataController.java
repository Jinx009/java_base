...
@Controller
@RequestMapping(value = "/interface/boken")
public class InterfaceBoKenDataController extends BaseController{
	private static final Logger logger = LoggerFactory.getLogger(InterfaceBoKenDataController.class);
	@Autowired
	...
	public Resp<?> device(){
		Resp<?> resp = new Resp<>(false);
		try {
			String token  = getToken(AppInfo.BOKEN.getAppId());
			if(token!=null){
