...
@Controller
@RequestMapping(value = "/interface/chengdu")
public class InterfaceChengduDataController extends BaseController{
private static final Logger logger = LoggerFactory.getLogger(InterfacePuruanDataController.class);
	...
	public Resp<?> device(){
		Resp<?> resp = new Resp<>(false);
		try {
			String token  = getToken(AppInfo.CHENGDU_TEST.getAppId());
			if(token!=null){
