...
@Controller
@RequestMapping(value = "/interface/bearhunting")
public class InterfaceBearHuntingDataController extends BaseController{

	private static final Logger log = LoggerFactory.getLogger(InterfaceBearHuntingDataController.class);
	...
	public Resp<?> device() {
		Resp<?> resp = new Resp<>(false);
		try {
			String token = getToken(AppInfo.BEAR_HUNTING.getAppId());
			if (token != null) {
