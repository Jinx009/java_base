...
@Controller
@RequestMapping(value = "/interface/shangcheng")
public class InterfaceShangchengDataController extends BaseController{
	...
	public Resp<?> device() {
		Resp<?> resp = new Resp<>(false);
		try {
			String token = getToken(AppInfo.HANGZHOU_SHANGCHENG.getAppId());
			if (token != null) {
