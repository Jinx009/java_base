...
@Controller
@RequestMapping(value = "/home/d/area")
public class AreaDataController extends BaseController{

private static final Logger log = LoggerFactory.getLogger(AreaDataController.class);
	...
	@ResponseBody
	public Resp<?> areas(Integer id){
		Resp<?> resp = new Resp<>(false);
		try {
			String token  = getMyToken();
			if(token!=null){
				String result = httpService.get(HttpData.areaListeUrl(id,token));
	...
	@ResponseBody
	public Resp<?> areaUpdate(Integer id,String name,String desc){
		Resp<?> resp = new Resp<>(false);
		try {
			String token  = getMyToken();
			if(token!=null){
				String result = httpService.postParams(HttpData.areaUpdateUrl(token,id,name,desc));
				JSONObject jsonObject = JSONObject.parseObject(result);
				if(!BaseConstant.HTTP_ERROR_CODE.equals(result)){
					if(BaseConstant.HTTP_OK_CODE.equals(jsonObject.getString(BaseConstant.RESP_CODE))){
						HttpData.mergeOrgan(getMofangSessionId(),2,name,"VALID");
						resp = new Resp<>(BaseConstant.HTTP_OK_CODE,BaseConstant.HTTP_OK_MSG,jsonObject.get(BaseConstant.PARAMS));
					}else{
						resp = new Resp<>(BaseConstant.HTTP_ERROR_CODE,jsonObject.getString(BaseConstant.MSG),jsonObject.getString(BaseConstant.PARAMS));
					}
