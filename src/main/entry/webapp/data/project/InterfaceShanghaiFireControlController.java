...
@Controller
@RequestMapping(value = "/interface/sfc")
public class InterfaceShanghaiFireControlController extends BaseController {
	...
	public Resp<?> device() {
		Resp<?> resp = new Resp<>(false);
		try {
			String token = getToken(AppInfo.SHANGHAI_FIRE_CONTROL.getAppId());
			if (token != null) {
