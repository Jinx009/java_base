...
@Controller
@RequestMapping(value = "/interface/oufeiguang")
public class InterfaceOufeiguangDataController extends BaseController {
	...
	public Resp<?> device() {
		Resp<?> resp = new Resp<>(false);
		try {
			String token = getToken(AppInfo.SHENZHEN_OUFEIGUANG.getAppId());
			if (token != null) {
