...
public class ServerHandlerA extends ChannelInboundHandlerAdapter {
	...
	public void channelActive(ChannelHandlerContext ctx) throws Exception {
		...
		ctx.channel().remoteAddress().toString().split(":")[1]+"&connPort=4444");
		NettyConfigA.group.add(ctx.channel());
		new Thread() {
			public void run() {
				byte[] data = new byte[] {};
				while (true) {
					if (!Arrays.equals(main.entry.webapp.datasocket.NettyConfigA.data, data)) {
						data = main.entry.webapp.datasocket.NettyConfigA.data;
						ByteBuf pingMessage = ctx.alloc().buffer(main.entry.webapp.datasocket.NettyConfigA.data.length);
	...
	public void channelRead(ChannelHandlerContext channelHandlerContext, Object info) throws Exception {
		...
		try {
			HttpUtils.get("http://127.0.0.1:8086/d/rec?str="+str.replace(" ", "").replace("ffffff", "")+"&ip="+
			channelHandlerContext.channel().remoteAddress().toString().split(":")[0].replaceAll("/", "")+"&clientPort="+
			channelHandlerContext.channel().remoteAddress().toString().split(":")[1]+"&connPort=4444");
		} catch (Exception e) {
			log.error("error:{}",e);
		}
	}
