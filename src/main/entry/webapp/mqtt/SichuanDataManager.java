...
@Component
public class SichuanDataManager {
	...
	private static SichuanDataManager dm;
	...
	public void start() {
		try {
			init();
			client = new MqttClient(SichuanMqttUtils.HOST,SichuanMqttUtils.SERVER_CLINETID, new MemoryPersistence());
			options = new MqttConnectOptions();
			options.setCleanSession(false);
			options.setUserName(SichuanMqttUtils.USERNAME);
			options.setPassword(SichuanMqttUtils.PASSWORD.toCharArray());
			options.setConnectionTimeout(10);
			options.setKeepAliveInterval(20);
			client.setCallback(new SichuanPushCallback());
			...
			log.warn("sub mac:{}", SichuanMqttUtils.SERVER_CLINETID);
		} catch (Exception e) {
			log.error("e:{}", e);
		}
	}
