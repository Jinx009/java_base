...
public class StatusCheckTask {
	...
	private void saveVedio(ParkingVedio parkingVedio) {
		...
		try {
			...
			String sql = "insert into record_download_info (iSerialNo,tCreateTime,sCameraIndex,sPlateNo,tBeginTime,tEndTime,sFileSavePath,sFtpUserName,sFtpPassWord,tUpdateTime,iUploadStatus,sUploadMsg,iFileSize) "
					+ "values(?,?,?,?,?,?,?,?,?,?,?,?,?)";
			log.warn("Connect Sql update Success!:{}", sql);
			Date date = sdf2.parse(parkingVedio.getEventTime());
			String beginTime = sdf.format(new Date(date.getTime()-30000));
			String endTime = sdf.format(new Date(date.getTime()+30000));
			Date date2 = sdf2.parse(parkingVedio.getChangeTime());
			String filePath = "ftp://10.0.0.1.18/"+sdf3.format(date2)+"/"+parkingVedio.getMac()+"_"+parkingVedio.getChangeTime()+"_";
			...
			pstmt.setInt(1, parkingVedio.getId());
			pstmt.setString(2, String.valueOf(new Date().getTime()));
			pstmt.setString(3, parkingVedio.getCameraIndex());
			pstmt.setString(4, "");
			pstmt.setString(5, beginTime);
			pstmt.setString(6, endTime);
			pstmt.setString(7, filePath);
			pstmt.setString(8,"ftp_user1");
			pstmt.setString(9, Base64.getEncoder().encodeToString("Zhanway2017".getBytes(StandardCharsets.UTF_8)));
			pstmt.setString(10, "");
			pstmt.setInt(11, 0);
			pstmt.setString(12, "");
			pstmt.setInt(13,0);
