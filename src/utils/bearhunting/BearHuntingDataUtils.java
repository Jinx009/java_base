...
public class BearHuntingDataUtils {
	...
	public static void main(String[] args) {
		...
		log.setAvailable(0);
		log.setMac("0001180316000001");
		System.out.print(sendStatusFire(KeyUtils.STATUS_FIRE_URL2, log,"zhanway"));
	}

	public static String sendStatusFire(String url, LogSensorStatus s, String routerMac) {
		String res = "{\"status\":0}";
		try {
			Date date = new Date();
			...
			String sendParams = params + "&sign=" + KeyUtils.get2(params);
			log.warn("[bearhunting status: params{}]", sendParams);
			res = HttpUtil.sendPost(url, sendParams);
		} catch (Exception e) {
			log.error("[bearhunting status error:{}]", e);
