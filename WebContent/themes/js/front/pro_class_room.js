...
function _getData() {
	...
	$.ajax({
		...
		success : function(res) {
			...
			if (_a == 2) {
				$('#time1').removeClass('button-danger').addClass('disabled').attr('onclick', '').html('上午 [可预订]');
			}
			if (_a == 1) {
		...
		if(_type!=2&&_type!=3){
			...
			if(!reg.test(_num)){
				...
				layer.open({
				    ...
				    ,skin: 'footer'
				    ,yes: function(index){
				       var _params = 'type=1&orderTime='+_orderTime+'&userId='+_userId+'&num='+_num;
				       $.ajax({
				    	   url:'/d/order/saveOrder',
				    	   data:_params,
				    	   dataType:'json',
				    	   type:'post',
				    	   success:function(res){
